...
}),userSelect.select2("val","");break;case"role":$.each(roleSelect.select2("data"),function(index,val){selection.push({type:"role",user:val.id,label:val.text}),roleSelect.find('option[value="'+val.id+'"]').remove()}),roleSelect.select2("val","")}$.each(selection,function(index,val){$(body).append(lineTpl(val))})},_disableAccessOnGrant=function(){var $managersChecked=$("#permissions-table").find(".privilege-GRANT:checked"),$cantAccess=$managersChecked.closest("tr").find(".privilege-WRITE"),$managers=$("#permissions-table").find(".privilege-GRANT").not(":checked"),$canAccess=$managers.closest("tr").find(".privilege-WRITE");$canAccess.removeClass("disabled").closest("label").tooltipster("disable"),$cantAccess.addClass("disabled").closest("label").tooltipster("disable")
},mainCtrl={start:function(){var $container=$(".permission-container"),$form=$("form",$container),$submiter=$(":submit",$form);_preventManagerRemoval(),_disableAccessOnGrant(),userSelect=$("#add-user").select2(),roleSelect=$("#add-role").select2(),$("#add-user-btn").on("click",function(event){event.preventDefault(),_addPermission("user")}),$("#add-role-btn").on("click",function(event){event.preventDefault(),_addPermission("role")}),$("#permissions-table").on("click",".privilege-GRANT:not(.disabled) ",function(){if($(this).is(":checked")!=[]){var accessCheckbox=$(this).closest("tr").find(".privilege-WRITE").not(":checked")[0];$(accessCheckbox).click()}_preventManagerRemoval(),_disableAccessOnGrant()
}).on("click",".delete_permission:not(.disabled)",function(event){event.preventDefault(),_deletePermission(this)}),$form.on("submit",function(e){e.preventDefault(),e.stopImmediatePropagation()}),$submiter.on("click",function(e){e.preventDefault(),$submiter.addClass("disabled"),$.post($form.attr("action"),$form.serialize()).done(function(res){res&&res.success?feedback().success(__("Permissions saved")):feedback().error(__("Something went wrong..."))}).complete(function(){$submiter.removeClass("disabled")})})}};return mainCtrl});
