...
$.ajax({url:$form.attr("action"),type:"POST",data:$form.serialize(),global:!1}).done(function(data){feedback().success(__("Permissions saved")),data&&data.tokenName&&data.token&&$('[name="'+data.tokenName+'"]',$form).val(data.token)}).fail(function(jqXHR,textStatus,errorThrown){var error=httpErrorParser.parse(jqXHR,textStatus,errorThrown);feedback().error(error.message)}).complete(function(){$submitter.removeClass("disabled")})}})}};return mainCtrl});
