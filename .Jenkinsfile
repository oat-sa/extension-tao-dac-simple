    ...
    stages {
        stage('Resolve TAO dependencies') {
            environment {
                ...
                REPO_NAME='oat-sa/extension-tao-dac-simple'
            }
            steps {
                sh(
        ...
        stage('Tests') {
            parallel {
                stage('Backend Tests') {
                    when {
                        expression {
                            fileExists('build/taoDacSimple/test/unit')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build'){
                            sh(
                                ...
                                script: './vendor/bin/phpunit taoDacSimple/test/unit'
                            )
                        }
                    }
                }
                stage('Frontend Tests') {
                    when {
                        expression {
                            fileExists('build/taoDacSimple/views/build/grunt/test.js')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build/tao/views/build') {
                            ...
                            sh (
                                ...
                                script: 'npx grunt connect:test taodacsimpletest'
                            )
                        }
                    }
                }
